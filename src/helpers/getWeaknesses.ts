...
import { ElementType, Multipliers } from '@/models/types';

export default function getWeaknesses(types: ElementType[]) {
  ...
  types.forEach((type) => {
    ...
    no_damage_from.forEach((type) => {
      defense[type] = defense[type] ?? 0;
      defense[type] = defense[type] * 0;
    });

    half_damage_from.forEach((type) => {
      defense[type] = defense[type] ?? 0.5;
      defense[type] = defense[type] * 0.5;
    })

    double_damage_from.forEach((type) => {
      defense[type] = defense[type] ?? 2;
      defense[type] = defense[type] * 2;
    })
  })

  return Object.keys(defense).filter((type) => defense[type as ElementType]! === 2);
};